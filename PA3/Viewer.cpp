...
void Viewer::Motion(int x, int y)
{
	...
	if (m_Mouse_Event[0])
	{
		if (!interationMode)
		{
			...
			if (m_Rotate[0] >= 360 || m_Rotate[0] < -360)
				m_Rotate[0] = 0;
			if (m_Rotate[1] >= 360 || m_Rotate[1] < -360)
				m_Rotate[1] = 0;
		}
		else if (interationMode)
		{
			//191016
			vec3 mouse_force = vec3(diffx, -diffy, 0);
			mouse_force = mouse_force.rotate(1, -m_Rotate[1]);
	...
	//printf("mouse_force : %f, %f, %f\n", S_Simulator.external_force.x, S_Simulator.external_force.y, S_Simulator.external_force.z);
	glutPostRedisplay();
}

void Viewer::PassiveMotion(int x, int y)
{
	S_Simulator.external_force = vec3(0, 0, 0);
	//printf("mouse_force : %f, %f, %f\n", S_Simulator.external_force.x, S_Simulator.external_force.y, S_Simulator.external_force.z);
